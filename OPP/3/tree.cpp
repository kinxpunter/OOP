...
void btree::insert(const std::shared_ptr<Figure>& figure,int key, node *leaf)
{
     std::shared_ptr<TreeItem> item = std::make_shared<TreeItem>(figure);
...
void btree::insert(const std::shared_ptr<Figure>& figure,int key)
{
     std::shared_ptr<TreeItem> item = std::make_shared<TreeItem>(figure);
  if(root!=NULL)
...
std::ostream& operator << (std::ostream& os, const btree& btree)
{
	...
	{
		std::shared_ptr<TreeItem> item = btree.root->value;

		while (item != NULL)
		{
			os << item->getFigure()->print();
			item = item->getNext();
		}
	}
